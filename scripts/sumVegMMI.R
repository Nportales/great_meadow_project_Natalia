...
sumVegMMI <- function(site = "all", panel = NA, years = 2013:2023,
  ...
  if (!all(is.na(panel))) {
    if (!is.numeric(panel)) {
      stop("`panel` must be numeric or integer.")
    }
    if (!all(panel %in% c(1, 2, 3, 4, -1))) {
      stop("`panel` must be a subset of 1, 2, 3, 4, or -1.")
    }
  }
  ...
  strata <- get("species_by_strata_tlu", envir = env)[,c("Code", "Location_ID", "Visit_ID", "Panel", "Date", "Year", "Visit_Type",
  ...
  if (all(is.na(vmmi_year$Panel))) {
    warning("All Panel values are NA. Skipping panel filtering.")
    vmmi_panel <- vmmi_year
  } else {
  ...
  vmmi_final <- left_join(vmmi_qaqc, loc, by = c("Code", "Location_ID")) |>
