...
RAM_stressors <- read.csv("data/raw_data/vegetation_data/all_veg_data/RAM_stressors_all_2012_2025.csv") %>%
  as_tibble()

vertical_complexity <- read.csv("data/raw_data/vegetation_data/all_veg_data/vertical_complexity_all_2012_2025.csv") %>%
  as_tibble()

## NETN SEN site data (already processed) ##

VMMI_NETN_SEN <- read.csv("data/processed_data/vegetation_data/NETN_vegMMI_allsites_2011-2025.csv") %>%
  as_tibble()
...
sumVegMMI <- function(site = "all", panel = c(-1, 1:4), years = 2012:format(Sys.Date(), "%Y"),
                      QAQC = FALSE){
  ...
  loc <- get("locations", envir = env)[,c("Code", "Location_ID", "Panel", "xCoordinate", "yCoordinate", "Latitude", "Longitude", "UTM_Zone")]
  ...
  vmmi_final <- left_join(vmmi_qaqc, loc, by = c("Code", "Location_ID", "Panel")) |>
    select(Code, Location_ID, Visit_ID, Panel, xCoordinate, yCoordinate, Latitude, Longitude, UTM_Zone, Date, Year, Visit_Type,
...
# view output
VMMI_result <- sumVegMMI()
print(VMMI_result)


## add SEN site veg data ##

VMMI_result_updated <- bind_rows(
  VMMI_result,
  VMMI_NETN_SEN %>%
    filter(local_id == "GILM" | str_detect(Code, "304")) %>%
    select(any_of(names(VMMI_result)))
) %>% 
  mutate(
    Latitude = case_when(
      str_detect(Code, "304") ~ 44.36275,
      TRUE ~ Latitude
    ),
    Longitude = case_when(
      str_detect(Code, "304") ~ -68.27162,
      TRUE ~ Longitude
    )
  )

# Save outputs as CSV
# write.csv(VMMI_result_updated, "data/processed_data/vegetation_data/FOA_NETN_VMMI_2011_2025_20260324.csv", row.names = FALSE)



## Graveyard ## ----------------------------------------------------------------
# 
# sumVegMMI <- function(site = "all", panel = -1, years = 2015:2025,
#                       QAQC = FALSE){
#   
#   #---- Error Handling ----
#   # Make more general for Non-NETN sites
...
# 
# # Save outputs as CSV
# # write.csv(VMMI_result, "data/processed_data/vegetation_data/FOA_VMMI_2015_2025_20260310.csv", row.names = FALSE)
