...
calculate_wetland_significance <- function(data, selected_years, alpha = 0.05) {
  # Filter data for selected years and group by wetland
  filtered_data <- data %>%
...
server <- function(input, output, session) {
  ...
  # Reactive for significance testing results
  significance_results <- reactive({
    req(input$stats_year)
    calculate_wetland_significance(wl_stats, input$stats_year, alpha = 0.05)
  })
  ...
  output$wl_stats <- DT::renderDataTable({
    ...
    # Add significance highlighting when in multi-year mode and comparing wetlands
    if (input$time_summary == "multi" && length(input$stats_year) > 1) {
